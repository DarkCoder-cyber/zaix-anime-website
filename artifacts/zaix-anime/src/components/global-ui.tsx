      ...
      {open && (
        <div className="w-[92vw] max-w-sm sm:w-80 bg-black/95 border border-primary/30 rounded-2xl shadow-neon-intense overflow-hidden mb-3">
          <div className="px-4 py-3 border-b border-primary/20 flex items-center justify-between">
            ...
            <button className="text-muted-foreground hover:text-white" onClick={() => setOpen(false)}>×</button>
          </div>
          <div className="h-56 overflow-y-auto p-3 space-y-2 text-sm hide-scrollbar">
            {messages.map((m) => <div key={m.id} className="bg-secondary/60 rounded-xl px-3 py-2"><span className="text-primary font-semibold">{m.user}:</span> <span className="text-white">{m.text}</span></div>)}
          </div>
          <div className="p-3 border-t border-primary/15 flex gap-2">
            <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Message the room..." className="flex-1 bg-secondary border border-border rounded-full px-3 py-2 text-sm text-white focus:outline-none focus:border-primary/60" />
            <button onClick={() => { if (!input.trim()) return; setMessages((m) => [...m, { id: Date.now(), user: "You", text: input.trim() }]); setInput(""); }} className="w-10 h-10 rounded-full bg-primary text-black shadow-neon flex items-center justify-center">↗</button>
          </div>
        </div>
      )}
