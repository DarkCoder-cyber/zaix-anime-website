...
import { Search, History, Sparkles, BookOpen } from "lucide-react";
    ...
    <main className="min-h-screen bg-background pb-20">
      <section className="relative w-full h-[80vh] min-h-[560px] flex items-center justify-center overflow-hidden">
        ...
        <div className="container mx-auto px-4 z-10 text-center flex flex-col items-center mt-16">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-center w-full max-w-md">
            <Button size="lg" className="w-full sm:w-auto bg-primary text-black hover:bg-primary/90 shadow-neon font-bold text-lg h-14 px-8" onClick={() => setActiveTab("anime")}>Browse Anime</Button>
            <Button size="lg" variant="outline" className="w-full sm:w-auto border-primary/50 text-white hover:bg-primary/10 hover:text-primary backdrop-blur-sm h-14 px-8" onClick={() => setActiveTab("manga")}>Read Manga</Button>
          </div>
        </div>
      </section>
      ...
      <section className="py-4 border-b border-border/50 bg-black/50 backdrop-blur-md sticky top-16 z-40">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide pb-1">
            {TAB_CONFIG.map((tab) => (
              <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`whitespace-nowrap px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-200 ${activeTab === tab.key ? "bg-primary text-black shadow-neon" : "border border-primary/30 text-foreground/80 hover:bg-primary/10 hover:text-primary hover:border-primary"}`}>
                {tab.emoji} {tab.label}
              </button>
            ))}
            <div className="h-5 w-px bg-border mx-2 shrink-0" />
            {CATEGORIES.slice(0, 5).map((category) => (
              <button key={category} className="whitespace-nowrap px-4 py-2 rounded-full border border-white/10 text-sm text-foreground/60 hover:bg-primary/10 hover:text-primary hover:border-primary/40 transition-all">{category}</button>
            ))}
          </div>
        </div>
      </section>
      ...
      {continueWatching.length > 0 && (
        <section className="py-14 container mx-auto px-4">
          <SectionHeader icon={<History className="w-5 h-5 text-primary" />} title="Continue Watching" subtitle="Pick up where you left off" />
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {continueWatching.map((item) => (
              <button key={`${item.id}-${item.visitedAt}`} onClick={() => setLocation(`/watch/${item.id}`)} className="text-left group">
                <div className="rounded-xl overflow-hidden border border-primary/20 hover:border-primary/60 transition-all shadow-lg group-hover:shadow-neon">
                  <div className="aspect-[3/4] bg-secondary relative">
                    {item.image ? <img src={item.image} alt={item.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><BookOpen className="w-10 h-10 text-primary/40" /></div>}
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent" />
                    <span className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-primary text-black text-[10px] font-bold shadow-neon">Resume</span>
                  </div>
                </div>
                <p className="mt-2 text-sm font-semibold text-white line-clamp-2 group-hover:text-primary">{item.title}</p>
              </button>
            ))}
          </div>
        </section>
      ...
      {(recommendedAnime.length > 0 || recommendedManga.length > 0) && (
        <section className="py-10 container mx-auto px-4">
          <SectionHeader icon={<Sparkles className="w-5 h-5 text-primary" />} title="Recommended for You" subtitle="Based on your recent activity" />
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div>
              <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><span className="text-primary">🎌</span> Anime</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {recommendedAnime.length ? recommendedAnime.map((item) => (
                  <button key={item.id} onClick={() => setLocation(`/watch/${item.id}`)} className="text-left group">
                    <div className="rounded-xl overflow-hidden border border-primary/20 hover:border-primary/60 transition-all shadow-lg">
                      <div className="aspect-[3/4] bg-secondary relative">
                        {item.image ? <img src={item.image} alt={item.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><BookOpen className="w-10 h-10 text-primary/40" /></div>}
                      </div>
                    </div>
                    <p className="mt-2 text-xs font-medium text-white line-clamp-2 group-hover:text-primary">{item.title}</p>
                  </button>
                )) : <div className="text-sm text-muted-foreground col-span-full">Watch a few anime to get recommendations.</div>}
              </div>
            </div>
            <div>
              <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><span className="text-primary">📚</span> Manga</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {recommendedManga.length ? recommendedManga.map((item) => (
                  <button key={item.id} onClick={() => setLocation(`/manga/${item.id}`)} className="text-left group">
                    <div className="rounded-xl overflow-hidden border border-primary/20 hover:border-primary/60 transition-all shadow-lg">
                      <div className="aspect-[3/4] bg-secondary relative">
                        {item.image ? <img src={item.image} alt={item.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><BookOpen className="w-10 h-10 text-primary/40" /></div>}
                      </div>
                ...
                )) : <div className="text-sm text-muted-foreground col-span-full">Read a few manga to get recommendations.</div>}
              </div>
            </div>
          </div>
        </section>
        ...
        <>
          <section className="py-16 container mx-auto px-4">
            ...
            <div className="flex overflow-x-auto gap-6 pb-8 snap-x -mx-4 px-4 sm:mx-0 sm:px-0 custom-scrollbar">
              {trendingLoading ? Array.from({ length: 6 }).map((_, i) => <div key={i} className="w-[240px] sm:w-[280px] shrink-0"><Skeleton className="w-full aspect-[3/4] rounded-xl" /></div>) : trendingAnime.map((anime, i) => <div key={`trending-${anime.malId}-${i}`} className="w-[240px] sm:w-[280px] shrink-0 snap-start"><AnimeCard anime={anime} layout="trending" /></div>)}
            </div>
          </section>
          ...
          <section className="py-16 container mx-auto px-4 bg-gradient-to-b from-transparent to-primary/5 rounded-3xl mb-12">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {recentLoading ? Array.from({ length: 8 }).map((_, i) => <Skeleton key={i} className="w-full aspect-[3/4] rounded-xl" />) : recentAnime.map((anime, i) => <AnimeCard key={`recent-${anime.malId}-${i}`} anime={{ ...anime, isNew: true }} layout="new" />)}
            </div>
          </section>
        </>
      )}
