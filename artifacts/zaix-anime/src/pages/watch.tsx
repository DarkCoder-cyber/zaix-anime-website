...
export default function WatchPage() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        const yt = data?.data?.[0]?.trailerUrl
          || data?.data?.[0]?.trailer?.url
          || data?.data?.[0]?.trailer?.embedUrl
          || null;
  ...
  const currentProvider = streamData?.providers?.find(p => p.name === activeProvider)
    ?? streamData?.providers?.[0]
    ?? null;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <div className="w-full aspect-video bg-black rounded-xl overflow-hidden relative border border-primary/20 shadow-neon" style={{ transform: "translateZ(0)" }}>
              <div className="absolute inset-0 pointer-events-none opacity-30 bg-radial-gradient" />
              ...
              {!streamLoading && !streamError && !currentEmbedUrl && (
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  {anime?.image && (
                    <img src={anime.image} alt="Poster" className="absolute inset-0 w-full h-full object-cover opacity-20 blur-sm" />
                  )}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="outline" className="border-primary/30 text-primary hover:bg-primary/10" onClick={() => alert("Watch party coming soon!")}>
              ...
              <Button variant="outline" className="border-primary/30 text-primary hover:bg-primary/10" onClick={startPip} disabled={!currentEmbedUrl}>
                <PictureInPicture2 className="w-4 h-4 mr-2" /> {pipMode ? "PiP Active" : "Picture-in-Picture"}
              </Button>
            </div>
            ...
            {streamData && !streamLoading && !streamError && streamData.providers?.length > 0 && (
              <div className="flex flex-col gap-3 bg-card border border-border rounded-xl px-4 py-3 -mt-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">Streaming source — if one doesn't load, try another:</span>
                </div>
                <div className="flex flex-wrap gap-2">
                ...
                {animeLoading ? (
                  ...
                  <>
                    ...
                    {trailerUrl && (
                      <Button
                        variant="outline"
                        className="mt-2 border-primary/30 text-primary hover:bg-primary/10 w-fit"
                    ...
                    {showTrailer && trailerUrl && (
                      <iframe
                        className="mt-2 w-full aspect-video rounded-xl border border-primary/20"
                        src={trailerUrl}
                        allowFullScreen
                        title="Trailer"
                      />
    ...
    <div className="overflow-y-auto custom-scrollbar pr-1 flex flex-col gap-2" style={{ maxHeight }}>
      {episodes.map((ep) => (
        ...
        >
          ...
          {ep.filler && (
            <Badge variant="outline" className="text-[9px] border-yellow-500/50 text-yellow-500 bg-yellow-500/10 px-1 py-0 shrink-0">
              FILLER
            </Badge>
          )}
        </button>
